...
contract Valuer {
    ...
    function calculateComponentsValuation(address[] memory components, uint256[] memory componentQuantities, address masterQuoteAsset) external view returns (uint256[] memory) {
        IPriceOracle priceOracle = controller.getPriceOracle();
        uint256[] memory componentsValues = new uint256[](components.length); 
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            uint256 aggregateUnits = componentQuantities[i];

            // Normalize each position unit to preciseUnits 1e18 and cast to signed int
            uint256 unitDecimals = ERC20(component).decimals();
            uint256 baseUnits = 10 ** unitDecimals;
            uint256 normalizedUnits = aggregateUnits.preciseDiv(baseUnits);

            // Calculate valuation of the component. Debt positions are effectively subtracted
            uint256 value = normalizedUnits.preciseMul(componentPrice);
            componentsValues[i] = value;
        }
